...
public:
    explicit HikCameraNode(const rclcpp::NodeOptions & options) : Node("hik_camera", options)
    {
        ...
        for (unsigned int i = 0; i < device_list.nDeviceNum; ++i) {
            ...
            RCLCPP_INFO(
                this->get_logger(), "Camera[%u]: serial='%s' user='%s'", i, serial.c_str(),
                user_name.c_str());
        ...
        if (!found_requested_camera) {
            RCLCPP_FATAL(
                this->get_logger(), "Camera serial '%s' not found", camera_serial_number_.c_str());
        ...
        if (camera_serial_number_.empty()) {
            ...
            RCLCPP_INFO(
                this->get_logger(), "Opening camera serial '%s'", camera_serial_number_.c_str());
        }
...
private:
    void declareParameters()
    {
        ...
        if (MV_OK != status) {
            RCLCPP_WARN(
                this->get_logger(), "Failed to set AcquisitionFrameRateEnable, status = %d",
                status);
        } else {
